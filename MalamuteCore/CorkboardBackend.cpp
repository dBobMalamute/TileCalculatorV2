...
{
    QColor color = c.getCasing(PlugType::DataOut)->casing().getOutPortColor(c.getPlugIndex(PlugType::DataOut));
    c.getCasing(PlugType::DataIn)->casing().setInPortColor(c.getPlugIndex(PlugType::DataIn), color);

    connect(&c, &WireBackend::wireBackendMadeIncomplete, this,
            &CorkboardBackend::wireDeleted, Qt::UniqueConnection);
    connect(&c, &WireBackend::wireBackendMadeIncomplete, this,
            &CorkboardBackend::deleteWireSignals, Qt::UniqueConnection);
    connect(&c, &WireBackend::updated, this,
            &CorkboardBackend::updateWireSignals, Qt::UniqueConnection);
}

void CorkboardBackend::deleteWireSignals(const WireBackend &c)
{
    c.getCasing(PlugType::DataIn)->casing().setInPortToDefaultColor(c.getPlugIndex(PlugType::DataIn));
}

void CorkboardBackend::updateWireSignals(const WireBackend &c)
{
    QColor color = c.getCasing(PlugType::DataOut)->casing().getOutPortColor(c.getPlugIndex(PlugType::DataOut));
    c.getCasing(PlugType::DataIn)->casing().setInPortColor(c.getPlugIndex(PlugType::DataIn), color);
}
